...
sine_grating_motion_shader = [
    ...
    """ #version 140
        ...
        void main(void){
            ...
            } else if (iscorrectbout == -2) {color = mix(vec3(1.0,1.0,0.0), color, circle(st,indicator_center,0.005))};
    ...
    def update_stimulus(self, fish_index, stimulus_index, stimulus_time, dt):
        ...
        if 0 <= stimulus_time < 5:  # 0-5 rest; 5-30 CL; 30-120 asymmetric gain
            ...
            if stimulus_index == 0:
                ...
                elif ((fish_ac > 0) & (self.decided[fish_index] == 0)) & ((self.time_after_start[fish_index] > 0.05) & (self.int_angle[fish_index]/self.frame_count_after_start[fish_index] > (self.baseline_angle[fish_index] + 2))):
                    gain = fullgain*0.2
                ...
                ## During bout, decision time: indifferent
                elif ((fish_ac > 0) & (self.decided[fish_index] == 0)) & ((self.time_after_start[fish_index] > 0.05) & ((self.baseline_angle[fish_index] - 2) <= self.int_angle[fish_index]/self.frame_count_after_start[fish_index] <= (self.baseline_angle[fish_index] + 2))):
                    gain = fullgain*0.2
                    self.decided[fish_index] = 1
                    self.iscorrectbout[fish_index] = -2
