...
sine_grating_motion_shader = [
    ...
    """ #version 140
        ...
        void main(void){
            ...
            // compensatory
            } else if (condition == -1.0) {color = mix(vec3(0.0), color, circle(st,condition_indicator_center,0.005));
            // rest
...
class MyApp(Panda3D_Scene):
    def __init__(self, shared):
        ...
        self.stimulus_subtrialpertrial = 5
        self.stimulus_time_per_stimulus = 180*self.stimulus_subtrialpertrial #180 #240
    ...
    def update_stimulus(self, fish_index, stimulus_index, stimulus_time, dt):
        ...
        ######
        self.advised_stimulus_index[fish_index] = self.index_sequence[fish_index][trial_num]
        ######
        ...
        if (fish_ac > 0):
            ...
            ## within bout
                ...
                if (self.time_after_start[fish_index] < decisiontime):
                    self.vigor_compensatory[fish_index] = self.int_vigor[fish_index] / self.frame_count_after_start[fish_index]
        ...
        if 0 <= stimulus_time % self.stimulus_time_per_subtrial < 5:  # 0-5 rest; 5-30 CL; 30-120 asymmetric gain
            ...
            if self.advised_stimulus_index[fish_index] == 0:
            ...
            elif self.advised_stimulus_index[fish_index] == 1:
                self.condition[fish_index] = 3 # lowgain condition
                gain = lowgain
            elif self.advised_stimulus_index[fish_index] == 2:
                self.condition[fish_index] = 4 # highgain condition
                gain = highgain
            else:
                raise ValueError("Invalid \"self.advised_stimulus_index\".")
            forward_speed = 0.1
        else:
...
#            fish_speed = 300
        ...
        ## out of bout but within the compensatory stage:
        if (fish_ac == 0) & (self.time_compensatory[fish_index] >= 0):
            ...
            self.condition[fish_index] = -1 ## in compensatory mode
        ...
        return [trial_num, trial_num_processed, stimulus_index, gain, forward_speed, fish_speed, self.pattern_offset[fish_index], dt, stimulus_time, stimname, curr_angle, curr_angle_mean, self.baseline_angle[fish_index], self.frame_count_after_start[fish_index], self.int_angle[fish_index], self.typeofbout[fish_index], self.advised_stimulus_index[fish_index]]
