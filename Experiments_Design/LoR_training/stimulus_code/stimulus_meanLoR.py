...
class MyApp(Panda3D_Scene):
    def __init__(self, shared):
        ...
        self.stimulus_time_per_stimulus = 120*5 #180 #240
        ...
        self.frame_count_after_start = [0 for _ in range(4)]
    ...
    def update_stimulus(self, fish_index, stimulus_index, stimulus_time, dt):
        ...
        if stimulus_time > self.stimulus_time_per_stimulus:
            self.subtrial_num[fish_index] += 1 # 5 subtrial?
        ...
        if 0 <= stimulus_time % self.stimulus_time_per_subtrial < 5:  # 0-5 rest; 5-30 CL; 30-150 asymmetric gain
            gain = 0.1
            forward_speed = 0
            stimname = 0#'rest'
            ...
            forward_speed = 0.1
            stimname = 1#'CL'
        elif 30 <= stimulus_time % self.stimulus_time_per_subtrial < 120: # condition
            if stimulus_index == 0:
                ##########
                if (fish_ac > 0) & (self.prev_ac[fish_index] == 0):
                    gain = 0.1 # reset
                    self.frame_count_after_start[fish_index] = 0 # reset
                    self.int_angle[fish_index] = 0 # reset
                elif (fish_ac > 0) & (self.frame_count_after_start[fish_index] == 40) & (self.int_angle[fish_index] > self.baseline_angle[fish_index]):
                    gain = 0.1*0.2
                elif (fish_ac > 0) & (self.frame_count_after_start[fish_index] == 40) & (self.int_angle[fish_index] <= self.baseline_angle[fish_index]):
                    gain = 0.1*1.8
                else:
                ...
                if (fish_ac > 0):
                    ...
                    self.int_angle[fish_index] += (1/40) * curr_angle
                ##########
...
#            fish_speed = 300
        ...
        return [trial_num, trial_num_processed, stimulus_index, gain, forward_speed, fish_speed, self.pattern_offset[fish_index], dt, stimulus_time, stimname, curr_angle, curr_angle_mean, self.baseline_angle[fish_index]]
